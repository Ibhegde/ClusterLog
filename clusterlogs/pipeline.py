...
CLUSTERING_DEFAULTS = {"w2v_size": 300, "w2v_window": 7, "min_samples": 1}
...
class Chain(object):
    ...
    def process(self):
        ...
        if comm_rank == 0:
            ...
            if (
                self.clustering_type == "similarity"
                and self.groups.shape[0] <= self.threshold
            ):
    ...
    def tokenization(self):
        self.df["tokenized_pattern"] = tokenize_messages(
            self.df[self.target].values, self.tokenizer_type
        )

    @safe_run
    def cleaning(self):
        cleaned_strings = clean_messages(self.df[self.target].values)
        cleaned_tokens = tokenize_messages(
            cleaned_strings,
            self.tokenizer_type,
    ...
    def remove_unique_tokens(self, tokens):
        ...
        cleaned_tokens = [
            [token for token in row if frequency[token] > 1] for row in tokens
        ]
    ...
    def clusters_description(self):
        self.result = pd.DataFrame.from_dict(
            [
                item
                for item in self.groups.groupby("cluster").apply(
                    func=self.clusters_regroup
                )
            ],
            orient="columns",
    ...
    def process_timings(self):
        if self.timings["group_equals"] != 0 and self.timings["regroup"] != 0:
            self.timings["group_equals"] -= self.timings[
                "regroup"
            ]  # group_equals contains regroup
