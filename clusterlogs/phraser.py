...
def extract_common_phrases(pattern, algorithm):
    dispatch = {
        ...
        "TopicalPageRank": partial(
            _extract_common_phrases_pke, algorithm="TopicalPageRank"
        ),
        "PositionRank": partial(_extract_common_phrases_pke, algorithm="PositionRank"),
        "MultipartiteRank": partial(
            _extract_common_phrases_pke, algorithm="MultipartiteRank"
        ),
    ...
    try:
        ...
        raise KeyError(
            f"Invalid keyword extraction method name: {key}! Available methods are: {tuple(dispatch.keys())}"
        )
...
def _extract_common_phrases_ngrams(pattern):
    ...
    return bi_finder.nbest(bigram_measures.pmi, 5) + tri_finder.nbest(
        trigram_measures.pmi, 5
    )
...
def _extract_common_phrases_rake(pattern):
    ...
    phrases = sorted(
        Rake.run(text, minFrequency=1, minCharacters=3, maxWords=5),
        key=lambda x: x[1],
        reverse=True,
    )
    if len(phrases) == 0:
